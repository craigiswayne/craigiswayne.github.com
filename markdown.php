<?php
//HOWTO: https://guides.github.com/features/mastering-markdown/
class GitHubMarkdownRender {
	...
	const GITHUB_PERSONAL_ACCESS_TOKEN = 'e671bd553d90fd3484d42bc28213918f31bc6a52';
	...
	private function getRequestedPageFilePath() {
		...
		$file_path = $_SERVER['QUERY_STRING'] ?: self::DOCUMENT_ROOT.$_SERVER['REQUEST_URI'];
		$file_path = trim($file_path);
		//$requestURI = trim($_SERVER['DOCUMENT_ROOT'].$_SERVER['REQUEST_URI']);
		$file_path = preg_replace('/\?.+$/','',$file_path);


		// request URI must end with self::MARKDOWN_EXT
		return (preg_match('/\\' . self::MARKDOWN_EXT . '$/',$file_path))
			? $file_path
			: false;
	}
