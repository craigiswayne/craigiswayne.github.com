...
Nexus.google.maps.geocoder		= Nexus.google.maps.geocoder 	|| null;
...
Nexus.google.maps.reverse_geocode = function(param){
	
	Nexus.google.maps.geocoder = Nexus.google.maps.geocoder || new google.maps.Geocoder();

	Nexus.google.maps.geocoder.geocode({'address': param['query']}, function(results, status) {
		...
		if (status == google.maps.GeocoderStatus.OK){
			...
			for(var i in results[0].address_components){
				...
				if(results[0].address_components[i].types[0] == "street_number"){
					nexus_address.street_number = results[0].address_components[i].long_name;
					continue;
				}
				
				if(results[0].address_components[i].types[0] == "route"){
					nexus_address.street_name = results[0].address_components[i].long_name;
					continue;				
				}
				
				if(results[0].address_components[i].types[0] == "sublocality_level_1"){
					nexus_address.suburb = results[0].address_components[i].long_name;
					continue;				
				}
				
				if(results[0].address_components[i].types[0] == "locality"){
					nexus_address.city = results[0].address_components[i].long_name;
					continue;				
				}
				
				if(results[0].address_components[i].types[0] == "administrative_area_level_1"){
					nexus_address.province = results[0].address_components[i].long_name;
					continue;				
				}
				
				if(results[0].address_components[i].types[0] == "country"){
					nexus_address.country = results[0].address_components[i].long_name;
					continue;				
				}
			...
			if(param.map){
				var marker_title = "";
				for(var j in nexus_address){
					marker_title += nexus_address[j] + " ";
				}
				var marker_data = {
					position: results[0].geometry.location,
					title: marker_title
				};
				param.map.remove_all_markers();
				param.map.add_marker(marker_data);
			}
...
Nexus.google.maps.ajax_reverse_geocode = function(param){
	
	//need to set a radius
